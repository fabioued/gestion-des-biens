...
const getStatus = (status, deleted) => {
  if (deleted) {
    return <span className="atbd-tag tag-danger tag-transparented ">Annulée</span>
  }
  else {
    if (status === 'initieeparAgent' || status === 'initieeparDirecteur') {
      ...
      return <span className="atbd-tag tag-warning tag-transparented ">Soummise</span>
    }
          ...
          <tbody>
            {missions && missions.map(mission => (
              <tr key={mission.IdMission}>
                ...
                <td>
                  <div className="orderDatatable-title">
                    {getStatus(mission.status, mission.deleted)}
                  </div>
                </td>
              </tr>
            ))}
